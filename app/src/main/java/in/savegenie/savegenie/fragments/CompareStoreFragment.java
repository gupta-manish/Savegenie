...
public class CompareStoreFragment extends Fragment
{
    ...
    Runnable getCompareStoreList = new Runnable()
    {
        ...
        public void run()
        {
            gcsliat = new GetCompareStoreListAsyncTask();

            gcsliat.execute(listId, "0");

            try
            {
                listResponse = gcsliat.get();
            }
            ...
            adapter = new CompareStoreActivityAdapter(context,
                    getItemIds(listResponse.storeList.size()),
                    listResponse.storeList);
            ...
            mHandler.post(new Runnable()
            {
                ...
                {
                    spinnerRL.setVisibility(View.INVISIBLE);
                    if (listResponse.storeList.size() != 0)
                    {
                        itemListView.setVisibility(View.VISIBLE);
                        itemListView.setAdapter(adapter);
                    }
